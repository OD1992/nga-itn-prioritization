# ============================================================
# 03_map_itn_priority.R
# NGA ITN Microstratification — Step 3
# Produce per-state maps showing URBAN ward ITN priority.
# Rural wards are drawn as muted background context (not coloured).
# ============================================================
#
# DEPENDS ON OUTPUT OF 02_pfpr_ranking_itn_priority.R
#   - <out_dir>/wards_pfpr_ranked.gpkg
#
# OUTPUTS  (under out_dir/maps)
#   - maps/<STATE>_itn_priority_map.png    One landscape PNG per state
#   - maps/_index.csv                      Per-state ward counts by priority
# ============================================================
...
wards_path <- file.path(out_dir, "wards_pfpr_ranked.gpkg")
...
# Maps focus on the urban decision: only the two urban categories are coloured.
# Rural wards are drawn as a muted neutral fill so the state extent is still visible.
URBAN_PRIORITY_LEVELS <- c(
  "Prioritized (urban moderate/high PfPR)",
  "Deprioritized (urban very-low/low PfPR)"
)
URBAN_PRIORITY_COLORS <- c(
  "Prioritized (urban moderate/high PfPR)"  = "#065A82",  # deep blue
  "Deprioritized (urban very-low/low PfPR)" = "#B45309"   # amber
)
...
build_state_map <- function(state_name) {
  ...
  n_kept    <- sum(urban_wards$urban_priority == "Prioritized (urban moderate/high PfPR)",   na.rm = TRUE)
  n_dropped <- sum(urban_wards$urban_priority == "Deprioritized (urban very-low/low PfPR)", na.rm = TRUE)
  n_rural   <- nrow(rural_wards)

  caption_txt <- sprintf(
    "Urban wards: %d  (Prioritized %d · Deprioritized %d) · Rural context: %d wards",
    n_urban, n_kept, n_dropped, n_rural
  )
  ...
  ggplot() +
    ...
    labs(
      title    = paste0(state_name, " — Urban ITN priority wards"),
      subtitle = "Urban wards only (≥ 80% urban population), ranked by PfPR within state",
      caption  = caption_txt
    ) +
...
message("Generating ", length(state_names), " state maps ...")
...
index_tbl <- wards |>
  ...
  mutate(
    n_urban_total = `Prioritized (urban moderate/high PfPR)` + `Deprioritized (urban very-low/low PfPR)`,
    pct_prioritized = round(100 * `Prioritized (urban moderate/high PfPR)` / pmax(n_urban_total, 1), 1)
  ) |>
